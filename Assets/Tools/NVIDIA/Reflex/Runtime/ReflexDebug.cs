...
namespace NVIDIA
{
    ...
    public class ReflexDebug : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                Reflex.NvReflex_Plugin_GetLatency(ref markers);
                Reflex.NvReflex_Plugin_GetSleepStatus(ref sleepStatusParams);
            }
        ...
        private void OnGUI()
        {
            if (m_Enabled)
            {
                GUILayout.BeginArea(new Rect { x = 10, y = 10, width = 340, height = 400 }, "Reflex Information", GUI.skin.window);
